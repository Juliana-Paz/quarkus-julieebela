...
import br.unitins.tp2.service.EstampaService;
...
public class EstampaResource {
    ...
    public Response buscarTodos(@QueryParam("page") @DefaultValue("0") int page,
        ...
        List<EstampaResponseDTO> lista = service.findAll(page, pageSize);
    ...
    @POST
    @RolesAllowed("Adm")
    public Response incluir(@Valid EstampaDTO dto) {
        EstampaResponseDTO criada = service.create(dto);
        URI location = UriBuilder.fromResource(EstampaResource.class)
                .path("{id}")
                .build(criada.id());
    ...
    @Path("/{id: \\d+}")
    @RolesAllowed("Adm")
    public Response alterar(@PathParam("id") Long id, @Valid EstampaDTO dto) {
